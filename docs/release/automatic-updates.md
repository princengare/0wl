...
0wl is currently approved and listed on Mozilla Add-ons at version `0.1.5`:

https://addons.mozilla.org/addon/7e6f3c1073eb4e24a37d/

The current source release target is `0.1.5`.
...
```sh
UPDATE_BASE_URL="https://downloads.example.org/0wl" XPI_FILE="0wl-0.1.5.xpi" npm run updates:manifest
```
...
https://downloads.example.org/0wl/0wl-0.1.5.xpi
```
